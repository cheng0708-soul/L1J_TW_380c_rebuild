...
// 在線人數怪物倍增設定
public static boolean ONLINE_SPAWN_MULTIPLIER_ENABLED;
public static int ONLINE_SPAWN_THRESHOLD;
public static int ONLINE_SPAWN_MULTIPLIER;
    ...
    public static boolean GM_TALK;
	public static boolean Attack_Mob_HP_Bar; // 攻擊顯示怪物血條
	public static boolean ALL_ITEM_SELL; // 全道具販賣 by 丫傑
	public static boolean SHOW_NPC_ID;
	...
	public static byte DropItemMinLv;//TODO 丟棄物品刪除道具
	...
	public static void load() {
		...
		try {
			...
			CACHE_MAP_FILES = Boolean.parseBoolean(serverSettings.getProperty("CacheMapFiles", "true"));
		...
		try {
			...
			Use_Show_Announcecycle = Boolean.parseBoolean(altSettings.getProperty("UseShowAnnouncecycle", "false")); // 循環公告 2/3by 丫傑
			Show_Announcecycle_Time = Integer.parseInt(altSettings.getProperty("ShowAnnouncecycleTime", "30")); // 循環時間 2/3 by 丫傑
			ALT_WHO_TYPE = Integer.parseInt(altSettings.getProperty("ALTWHOTYPE","1"));
			GM_TALK = Boolean.parseBoolean(altSettings.getProperty("GMSpeakName", "false")); //TODO ＧＭ使用公頻(&)顯示方式 2/3
			Attack_Mob_HP_Bar = Boolean.parseBoolean(altSettings.getProperty("AttackMobHPBar", "false")); // 攻擊顯示怪物血條
			ALL_ITEM_SELL = Boolean.parseBoolean(altSettings.getProperty("AllItemSell", "false"));// 全道具販賣 by 丫傑 end
			SHOW_NPC_ID = Boolean.parseBoolean(altSettings.getProperty("SHOWNPCID", "true"));
			RATE_AIN_TIME = Integer.parseInt(altSettings.getProperty("RateAinTime", "30"));//TODO 殷海薩的祝福
			RATE_AIN_OUTTIME = Integer.parseInt(altSettings.getProperty("RateAinOutTime", "30"));//TODO 殷海薩的祝福
			RATE_MAX_CHARGE_PERCENT = Integer.parseInt(altSettings.getProperty("RateMaxChargePercent", "200"));//TODO 殷海薩的祝福
			BONUS_STATS1 = Integer.parseInt(altSettings.getProperty("BONUS_STATS1", "25"));//調整能力值上限 by 丫傑 end
			BONUS_STATS2 = Integer.parseInt(altSettings.getProperty("BONUS_STATS2", "5"));//調整能力值上限 by 丫傑 end
			BONUS_STATS3 = Integer.parseInt(altSettings.getProperty("BONUS_STATS3", "25"));//調整能力值上限 by 丫傑 end
			Drop_Item = Boolean.parseBoolean(altSettings.getProperty("DropItem", "false"));//TODO　刪除丟棄物品 by bill00148
			DropItemMinLv = Byte.parseByte(altSettings.getProperty("DropItemMinLv", "200"));//TODO　刪除丟棄物品 by bill00148
			MaxHPMP = Boolean.parseBoolean(altSettings.getProperty("FullHPMP", "false"));//TODO 升級血魔滿
			DeleteFood = Boolean.parseBoolean(altSettings.getProperty("DeleteFood", "false"));//TODO 廣播扣飽食度
			Gamesleep = Integer.parseInt(altSettings.getProperty("Gamesleep", "30"));
			RATE_XP_PET = Double.parseDouble(altSettings.getProperty("PetRateXp","1.0"));//TODO 寵物經驗倍率	
			Pet_Max_LV = Integer.parseInt(altSettings.getProperty("PetMaxLV", "50"));//TODO 寵物最高等級設定
			GM_OVERHEARD = Boolean.parseBoolean(altSettings.getProperty("GM_OVERHEARD", "false")); // 密語頻道
			GM_OVERHEARD0 = Boolean.parseBoolean(altSettings.getProperty("GM_OVERHEARD0", "false")); // 一般頻道 
			GM_OVERHEARD4 = Boolean.parseBoolean(altSettings.getProperty("GM_OVERHEARD4", "false")); // 血盟頻道
			GM_OVERHEARD11 = Boolean.parseBoolean(altSettings.getProperty("GM_OVERHEARD11", "false")); // 組隊頻道
			GM_OVERHEARD13 = Boolean.parseBoolean(altSettings.getProperty("GM_OVERHEARD13", "false")); // 聯盟頻道
			String strWar;
...
// 在線人數怪物倍增設定
ONLINE_SPAWN_MULTIPLIER_ENABLED = Boolean.parseBoolean(altSettings.getProperty("OnlineSpawnMultiplierEnabled", "False"));
ONLINE_SPAWN_THRESHOLD = Integer.parseInt(altSettings.getProperty("OnlineSpawnThreshold", "2"));
ONLINE_SPAWN_MULTIPLIER = Integer.parseInt(altSettings.getProperty("OnlineSpawnMultiplier", "2"));
		}
		catch (Exception e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
