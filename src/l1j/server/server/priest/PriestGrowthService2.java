...
public final class PriestGrowthService2 {
    private static final Logger _log = Logger.getLogger(PriestGrowthService2.class.getName());
    private static final SecureRandom RNG = new SecureRandom();
    public enum Category { PRIEST, PRINCESS }

    private PriestGrowthService2() {}

    public static boolean isPriestItem(int itemId) { return hasStageRow("priest_item_stage_map", itemId); }
    public static boolean isPrincessItem(int itemId) { return hasStageRow("princess_doll_stage_map", itemId); }
    private static boolean hasStageRow(String table, int itemId) {
        try (Connection con = L1DatabaseFactory.getInstance().getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT 1 FROM " + table + " WHERE item_id=? LIMIT 1")) {
            ps.setInt(1, itemId);
        ...
        try (Connection con = L1DatabaseFactory.getInstance().getConnection()) {
            ...
            if (stage <= 0) {
                pc.sendPackets(new S_SystemMessage(
                        "這不是可以升階的" + (cat == Category.PRIEST ? "祭司" : "公主娃娃") + "。"));
                return false;
            }
            if (stage >= 5) {
                pc.sendPackets(new S_SystemMessage("已達最高階。"));
                return false;
            }

            final int nextItemId = getNextItemId(con, cat, stage + 1);
            if (nextItemId == 0) {
                pc.sendPackets(new S_SystemMessage("下一階設定遺失，請通知管理員。"));
                return false;
            }
            ...
            if (success) {
                ...
                if (tpl == null) {
                    pc.sendPackets(new S_SystemMessage("升級模板遺失：" + nextItemId));
                    return false;
                }

                target.setItem(tpl); // keep same objid
            ...
            try {
                pc.sendPackets(new S_SystemMessage("升級發生例外：" + e.getClass().getSimpleName()));
