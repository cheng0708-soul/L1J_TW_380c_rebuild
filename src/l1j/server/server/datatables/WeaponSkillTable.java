...
public class WeaponSkillTable {
	...
	private void fillWeaponSkillTable(ResultSet rs) throws SQLException {
		while (rs.next()) {
			...
			int winCha = 0;
			try {
				winStr = rs.getInt("win_str");
				winCon = rs.getInt("win_con");
				winDex = rs.getInt("win_dex");
				winInt = rs.getInt("win_int");
				winWis = rs.getInt("win_wis");
				winCha = rs.getInt("win_cha");
			} catch (Exception e) {
				// 若資料表尚未新增欄位，維持預設 0 不影響舊有環境
			}
			L1WeaponSkill weaponSkill = new L1WeaponSkill(weaponId, probability, fixDamage, randomDamage, area, skillId, skillTime, effectId,
					effectTarget, isArrowType, attr, winStr, winCon, winDex, winInt, winWis, winCha);
