...
public class DropTable {
	...
	public void dropShare(L1NpcInstance npc, List<L1Character> acquisitorList, List<Integer> hateList) {
		...
		// 個人掉寶加倍：決定擁有額外掉寶權的玩家 (吃了個人轉運符且取得此怪掉寶權)
		L1PcInstance personalLuckPc = null;
		if (totalHate > 0) {
			int randomHate = Random.nextInt(totalHate);
			int chance = 0;
			for (int i = hateList.size() - 1; i >= 0; i--) {
				chance += hateList.get(i);
				if (chance > randomHate) {
					L1Character cand = acquisitorList.get(i);
					if (cand instanceof L1PcInstance) {
						L1PcInstance pcCand = (L1PcInstance) cand;
						if (pcCand.hasSkillEffect(L1SkillId.STATUS_PERSONAL_LUCKY_CHARM)) {
							personalLuckPc = pcCand;
						}
					}
					break;
				}
		...
		// 如果有吃個人轉運符，而且這次又取得掉寶權，則額外進行一輪個人掉寶判定
		if (personalLuckPc != null) {
			int mobIdForLuck = npc.getNpcTemplate().get_npcId();
			List<L1Drop> luckDropList = _droplists.get(mobIdForLuck);
			if (luckDropList != null) {

				double droprateLuck = Config.RATE_DROP_ITEMS * 2; // 個人掉寶加倍
				if (droprateLuck <= 0) {
					droprateLuck = 0;
				}
				double adenarateLuck = Config.RATE_DROP_ADENA;
				if (adenarateLuck <= 0) {
					adenarateLuck = 0;
				}
				if ((droprateLuck > 0) || (adenarateLuck > 0)) {
					int itemIdLuck;
					int itemCountLuck;
					int addCountLuck;
					int enchantlvlLuck;
					int randomChanceLuck;
					for (L1Drop drop : luckDropList) {
						itemIdLuck = drop.getItemid();
						if ((adenarateLuck == 0) && (itemIdLuck == L1ItemId.ADENA)) {
							continue;
						}

						randomChanceLuck = Random.nextInt(0xf4240) + 1;
						double rateOfMapIdLuck = MapsTable.getInstance().getDropRate(npc.getMapId());
						double rateOfItemLuck = DropItemTable.getInstance().getDropRate(itemIdLuck);
						if ((droprateLuck == 0) || (drop.getChance() * droprateLuck * rateOfMapIdLuck * rateOfItemLuck < randomChanceLuck)) {
							continue;
						}

						double amountLuck = DropItemTable.getInstance().getDropAmount(itemIdLuck);
						int minLuck = (int) (drop.getMin() * amountLuck);
						int maxLuck = (int) (drop.getMax() * amountLuck);

						itemCountLuck = minLuck;
						addCountLuck = maxLuck - minLuck + 1;
						if (addCountLuck > 1) {
							itemCountLuck += Random.nextInt(addCountLuck);
						}
						if (itemIdLuck == L1ItemId.ADENA) {
							itemCountLuck *= adenarateLuck;
						}
						if (itemCountLuck <= 0) {
							continue;
						}
						if (itemCountLuck > 2000000000) {
							itemCountLuck = 2000000000;
						}

						enchantlvlLuck = drop.getEnchantlvl();

						L1ItemInstance luckItem = ItemTable.getInstance().createItem(itemIdLuck);
						luckItem.setCount(itemCountLuck);
						luckItem.setEnchantLevel(enchantlvlLuck);

						personalLuckPc.getInventory().storeItem(luckItem);
						personalLuckPc.sendPackets(new S_ServerMessage(143, npc.getName(), luckItem.getLogName())); // \f1%0が%1をくれました。
					}
				}
			}
		}
