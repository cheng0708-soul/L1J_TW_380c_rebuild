...
public class ShopTable {
	...
	private List<Integer> enumNpcIds() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT DISTINCT npc_id FROM shop UNION SELECT DISTINCT npc_id FROM shop_cn");
	...
	private void loadShops() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM shop WHERE npc_id=? UNION ALL SELECT * FROM shop_cn WHERE npc_id=? ORDER BY item_id");
