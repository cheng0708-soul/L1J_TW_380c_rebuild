...
public class L1Teleport {
	...
	public static void teleport(L1PcInstance pc, int x, int y, short mapId, int head, boolean effectable, int skillType) {
		// 釋放原本站位，避免殘留空氣牆
		try { pc.getMap().setPassable(pc.getLocation(), true); } catch (Throwable __e) {}
		...
		// 佔用新站位，與移動邏輯一致
		try { l1j.server.server.model.map.L1WorldMap.getInstance().getMap(mapId).setPassable(x, y, false); } catch (Throwable __e) {}
}
