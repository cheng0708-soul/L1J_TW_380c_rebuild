...
public class L1WeaponSkill {
	...
	public L1WeaponSkill(int weaponId, int probability, int fixDamage,
			...
			int winStr, int winCon, int winDex, int winInt, int winWis, int winCha) {
		_weaponId = weaponId;
	...
	public int getWinWis() {
		return _winWis;
	}

	public int getWinCha() {
		return _winCha;
	}
	...
	public static double getWeaponSkillDamage(L1PcInstance pc, L1Character cha,
		...
		// 魔法武器 +11 以上 魔法發動率加成（+11~+30）
		L1ItemInstance weapon = pc.getWeapon();
		if (weapon != null && weapon.getItemId() == weaponId) {
		...
		if (weaponSkill.getWinStr() != 0) {
			damage += pc.getStr() * weaponSkill.getWinStr();
		}
		if (weaponSkill.getWinCon() != 0) {
			damage += pc.getCon() * weaponSkill.getWinCon();
		}
		if (weaponSkill.getWinDex() != 0) {
			damage += pc.getDex() * weaponSkill.getWinDex();
		}
		if (weaponSkill.getWinInt() != 0) {
			damage += pc.getInt() * weaponSkill.getWinInt();
		}
		if (weaponSkill.getWinWis() != 0) {
			damage += pc.getWis() * weaponSkill.getWinWis();
		}
		if (weaponSkill.getWinCha() != 0) {
			damage += pc.getCha() * weaponSkill.getWinCha();
		}
