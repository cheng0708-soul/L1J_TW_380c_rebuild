...
public class L1BossSpawn extends L1Spawn {
	...
	// 讓外部可以取得這隻 BOSS 當前的排程時間（下一次出現或本輪出現時間）
	public Calendar getScheduleTime() {
		return _activeSpawnTime;
	}
	...
	private void spawnBoss(Calendar spawnTime, int objectId) {
		...
		// DB に次回出現時間を書き込む（spawnlist_boss.next_spawn_time）
		updateNextSpawnTime(spawnTime);
	...
	// spawnlist_boss.next_spawn_time を更新
	private void updateNextSpawnTime(Calendar spawnTime) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"UPDATE spawnlist_boss SET next_spawn_time=? WHERE npc_id=?");
			pstm.setTimestamp(1, new Timestamp(spawnTime.getTimeInMillis()));
			pstm.setInt(2, getNpcId());
			pstm.executeUpdate();
		} catch (Exception e) {
			_log.log(Level.WARNING, "Failed to update next_spawn_time for boss npcId=" + getNpcId(), e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
