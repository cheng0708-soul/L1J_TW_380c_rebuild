...
public class L1Character extends L1Object {
	 ...
	 */
		...
		// Use map's LOS (arrow pass) and compute heading from the *current* step.
		// Also try symmetric check to avoid rounding asymmetry.
		return __lineOfFire(getX(), getY(), tx, ty) || __lineOfFire(tx, ty, getX(), getY());
	}

	private boolean __lineOfFire(int sx, int sy, int tx, int ty) {
		int chx = sx;
		int chy = sy;
		for (int i = 0; i < 15; i++) {
			if (chx == tx && chy == ty) {
			...
			int h = __headingFrom(chx, chy, tx, ty);
			if (!getMap().isArrowPassable(chx, chy, h)) {
				return false;
			}
			// step one tile towards target
			chx += Math.max(-1, Math.min(1, tx - chx));
			chy += Math.max(-1, Math.min(1, ty - chy));
		}
		// If the loop exits without reaching target, treat as blocked
		return (chx == tx && chy == ty);
	}

	// Same rounding as targetDirection(...) but with explicit (fromX,fromY)
	private int __headingFrom(int fromX, int fromY, int toX, int toY) {
		float dis_x = Math.abs(fromX - toX);
		float dis_y = Math.abs(fromY - toY);
		float dis = Math.max(dis_x, dis_y);
		if (dis == 0) return getHeading();
		int avg_x = (int)Math.floor((dis_x / dis) + 0.59f);
		int avg_y = (int)Math.floor((dis_y / dis) + 0.59f);
		int dir_x = 0, dir_y = 0;
		if (fromX < toX) dir_x = 1;
		if (fromX > toX) dir_x = -1;
		if (fromY < toY) dir_y = 1;
		if (fromY > toY) dir_y = -1;
		if (avg_x == 0) dir_x = 0;
		if (avg_y == 0) dir_y = 0;
		if ((dir_x == 0) && (dir_y == 1))  return 1;
		if ((dir_x == 1) && (dir_y == 1))  return 2;
		if ((dir_x == 1) && (dir_y == 0))  return 3;
		if ((dir_x == 1) && (dir_y == -1)) return 4;
	...
	public void clearSkillEffectTimer() {
	    ...
	    try {
	        for (L1SkillTimer timer : _skillEffect.values()) {
	            if (timer != null) { // 防止 null 造成 NPE
	                timer.end();
	            }
	        }
	    } catch (Exception e) {
	        // 這裡可以視情況要不要印 log，避免把玩家踢掉
	        // e.printStackTrace();
	    } finally {
	        _skillEffect.clear();
	    }
	...
	public L1Character getMobHPBar() {
		return _MobHPBar;
	}

	public void setMobHPBar(L1Character cha) {
		_MobHPBar = cha;
	}
	// end
	// 判斷特定狀態下才可攻擊 NPC
