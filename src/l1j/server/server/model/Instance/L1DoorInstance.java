...
private void setDoorPassable(int x, int y, boolean passable) {
	// 用 L1Location 版本避免不同核心 setPassable 簽名差異
	getMap().setPassable(new L1Location(x, y, getMapId()), passable);
}

/**
 * 依照門的寬度/方向，把門佔用到的所有格子即時切換成可通行/不可通行
 * - open 或 dead: passable=true
 * - close: passable=false
 */
private void updateDoorPassable(boolean passable) {
	if (getMap() == null) {
		return;
	}
	int entranceX = getEntranceX();
	...
	if (size == 0) { // 1 格門
		setDoorPassable(entranceX, entranceY, passable);
	} else { // 多格門（城門等）
		if (getDirection() == 0) { // ／向き：X 軸延伸，Y 固定
			for (int x = leftEdgeLocation; x <= rightEdgeLocation; x++) {
				setDoorPassable(x, entranceY, passable);
			}
		} else { // ＼向き：Y 軸延伸，X 固定
			for (int y = leftEdgeLocation; y <= rightEdgeLocation; y++) {
				setDoorPassable(entranceX, y, passable);
			}
		}
	}
}
...
public boolean isOpen() {
	return getOpenStatus() == ActionCodes.ACTION_Open;
}
