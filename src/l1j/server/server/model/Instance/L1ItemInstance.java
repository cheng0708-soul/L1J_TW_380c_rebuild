...
public class L1ItemInstance extends L1Object {
	...
	// 強化保護類型：0=無, 1=歸0防爆, 2=完全防爆
	private int _enchantProtectType = 0;
	...
	public int getEnchantProtectType() {
		return _enchantProtectType;
	}

	public void setEnchantProtectType(int enchantProtectType) {
		_enchantProtectType = enchantProtectType;
	}
	...
	public String getNumberedViewName(int count) {
		...
		if (itemType2 == 1) { // weapon
			...
			if (enchant >= 11) {
				...
				name.append("[爆率+").append(bonusLv).append("]");
				name.append("[爆傷+").append(bonusLv * 10).append("%]");
			}
		}
		// 高強化防具 +11 以上 額外後綴顯示（+11~+30）
		if (itemType2 == 2) { // armor
			int enchant = getEnchantLevel();
			if (enchant > 30) {
				enchant = 30;
			}
			if (enchant >= 11) {
				int bonusLv = enchant - 10; // +11 =1, +30 =20
				name.append("[+").append(bonusLv).append("傷害減免]");
			}
		}
		String __base = name.toString();
        __base = l1j.server.server.priest.PriestItemNameDecorator.decorate(this, __base);
        return __base;
	}
	...
	public String getNumberedName(int count) {
	    ...
	    if (isIdentified()) {
	        ...
	        if (getItem().getType2() == 1) { // 武器
	            ...
	            int attrEnchantKind  = getAttrEnchantKind();

	            if (attrEnchantLevel > 0 && attrEnchantKind > 0) {
	                String attrStr = "";

	                switch (attrEnchantKind) {
	                case 1: // 地
	                    switch (attrEnchantLevel) {
	                    case 1: attrStr = "[地之]"; break;
	                    case 2: attrStr = "[碎岩]"; break;
	                    case 3: attrStr = "[巨石]"; break;
	                    case 4: attrStr = "[地靈]"; break;
	                    case 5: attrStr = "[岩神]"; break;
	                    }
	                    break;
	                case 2: // 火
	                    switch (attrEnchantLevel) {
	                    case 1: attrStr = "[火之]"; break;
	                    case 2: attrStr = "[火海]"; break;
	                    case 3: attrStr = "[岩漿]"; break;
	                    case 4: attrStr = "[火靈]"; break;
	                    case 5: attrStr = "[火神]"; break;
	                    }
	                    break;
	                case 4: // 水
	                    switch (attrEnchantLevel) {
	                    case 1: attrStr = "[水之]"; break;
	                    case 2: attrStr = "[巨浪]"; break;
	                    case 3: attrStr = "[海嘯]"; break;
	                    case 4: attrStr = "[水靈]"; break;
	                    case 5: attrStr = "[水神]"; break;
	                    }
	                    break;
	                case 8: // 風
	                    switch (attrEnchantLevel) {
	                    case 1: attrStr = "[風之]"; break;
	                    case 2: attrStr = "[狂風]"; break;
	                    case 3: attrStr = "[颶風]"; break;
	                    case 4: attrStr = "[風靈]"; break;
	                    case 5: attrStr = "[風神]"; break;
	                    }
	                    break;
	                default:
	                    break;
	...
	public byte[] getStatusBytes() {
		...
		if (petItem != null) { // 寵物裝備
			...
			// 攻撃成功
			if (itemType2 == 1) { // weapon
				int hitMod = getItem().getHitModifier();
				// 高強化武器 +11 以上 額外攻擊成功顯示（只影響說明）
			...
			// 追加打撃
			if (itemType2 == 1) { // weapon
				int dmgMod = getItem().getDmgModifier();
				// 高強化武器 +11 以上 額外攻擊力顯示（只影響說明）
				int enchantLevel = getEnchantLevel();
				if (enchantLevel > 30) {
					enchantLevel = 30;
				}
			...
			// STR~CHA
			int addStr = getItem().get_addstr();
			int addDex = getItem().get_adddex();
			int addCon = getItem().get_addcon();
			int addWis = getItem().get_addwis();
			int addInt = getItem().get_addint();
			int addCha = getItem().get_addcha();

			// 高強化防具 +11 以上 顯示用能力加成（只影響說明，不影響實際能力）
			if (itemType2 == 2) { // armor
				int enchantLevel = getEnchantLevel();
				if (enchantLevel > 30) {
					enchantLevel = 30;
			...
			// 高強化武器 +11 以上 顯示用能力加成（只影響說明，不影響實際能力）
			if (itemType2 == 1) { // weapon
				int enchantLevel = getEnchantLevel();
				if (enchantLevel > 30) {
					enchantLevel = 30;
...
// HP, MP
			...
			// 火の属性 / 水 / 風 / 地
			int fire = getItem().get_defense_fire() + getFireMr();
			int water = getItem().get_defense_water() + getWaterMr();
			int wind = getItem().get_defense_wind() + getWindMr();
			int earth = getItem().get_defense_earth() + getEarthMr();

			// 高強化防具 +11 以上 額外屬性防禦顯示（顯示用，不影響實際能力）
			if (itemType2 == 2) { // armor
				int enchantLevel = getEnchantLevel();
				if (enchantLevel > 30) {
					enchantLevel = 30;
...
//6抗性
            final java.util.function.BiConsumer<Integer,Integer> writeAbRes = (typeId, val) -> {
...
// 凍結耐性
            writeAbRes.accept(1, getItem().get_regist_freeze());
// 石化耐性
            writeAbRes.accept(2, getItem().get_regist_stone());
// 睡眠耐性
            writeAbRes.accept(3, getItem().get_regist_sleep());
// 暗盲耐性
            writeAbRes.accept(4, getItem().get_regist_blind());
// 昏迷(スタン)耐性
            writeAbRes.accept(5, getItem().get_regist_stun());
// 束縛/支撐(ホールド)耐性
            writeAbRes.accept(6, getItem().get_regist_sustain());
			// MR
			int mr = getMr();
			// 高強化防具 +11 以上 額外魔法防禦顯示（顯示用，不影響實際能力）
			if (itemType2 == 2) { // armor
				int enchantLevel = getEnchantLevel();
				if (enchantLevel > 30) {
					enchantLevel = 30;
