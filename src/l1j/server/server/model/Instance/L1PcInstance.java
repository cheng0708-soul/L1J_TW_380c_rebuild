// L1PcInstance GOOD COPY (from your src.zip)
// SINGLE-FILE CLEAN PATCH v4 2025-09-08 00:13:48
// md5:1bb27f1907eb1a4fadb5f4cbc2d8f8a0  lines:4982
// L1PcInstance GOOD COPY (from your src.zip)
// CONSOLIDATED PATCH v3 2025-09-08 00:06:27
// md5:8d5ebc3557c4ce4f19dd5f285278aea5  lines:4979
/**
 *                            License
...
import l1j.server.server.utils.AutofarmHooks;
...
// === PShop currency selection flags ===
private boolean _shopModeChosen = false;
private boolean _shopModePendingOpen = false;
private int _privateShopCurrencyItemId = 40308; // default: Adena
// === end ===


    private int _shopMoneyType = 0; // 0=Adena, 1=Yuanbao

        // --- Private Shop currency (minimal, backward compatible) ---
	...
	public void logout() {
		// 釋放目前站位，避免留下空氣牆
		try { getMap().setPassable(getLocation(), true); } catch (Throwable __e) {}

        // AFK：角色狀態變更時，清除技能登錄模式
        this._afkSkillRegMode = false;
	...
	public void setTempID(int tempID) {
	 ...
	 // ==== 屬性卷二次確認 ====
    public int getPendingAttrEnchantObjId() {
        return _pendingAttrEnchantObjId;
    }

    public void setPendingAttrEnchantObjId(int id) {
        _pendingAttrEnchantObjId = id;
    }
	...
	public void death(L1Character lastAttacker) {
		...
		try { l1j.server.server.afk.AfkEventBridge.onDeath(this); } catch (Throwable __e) {}
}

	private class Death implements Runnable {
		private final L1Character lastAttacker;
		private Death(final L1Character lastAttacker) {
			this.lastAttacker = lastAttacker;
		}
		@Override
		public void run() {
			setCurrentHp(0);
			// ★ 掛機：死亡時立即強制關閉（只影響當前玩家自身的掛機狀態）
			try {
			AutofarmHooks.onDeath(L1PcInstance.this);
			} catch (Throwable __e) { /* 保持穩定性：吞例外 */ }
    ...
    // 屬性卷二次確認：記錄正在確認的武器物件 ID（0 表示沒有）
    private int _pendingAttrEnchantObjId;
...
// --- BEGIN: Dual-currency Private Shop fields/methods ---
/** 40308 = 金幣(ADENA), 240107 = 元寶 */
private int _tradeCurrency = 40308;
/** 是否已選擇商店貨幣（避免每次開店都彈窗） */
private boolean _shopCurrencyChosen = false;

public int getTradeCurrency() { return _tradeCurrency; }
public void setTradeCurrency(int id) { _tradeCurrency = id; _shopCurrencyChosen = true; }
public boolean isShopCurrencyChosen() { return _shopCurrencyChosen; }
public void resetShopCurrencyChosen() { _shopCurrencyChosen = false; _tradeCurrency = 40308; }

/** 依賣家的商店幣別檢查（私商店用） */
public boolean hasShopCurrency(int amount, l1j.server.server.model.Instance.L1PcInstance seller) {
    int cur = (seller != null) ? seller.getTradeCurrency() : 40308;
    return getInventory().checkItem(cur, amount);
}
/** 依賣家的商店幣別扣款（私商店用） */
public boolean payShopCurrency(int amount, l1j.server.server.model.Instance.L1PcInstance seller) {
    int cur = (seller != null) ? seller.getTradeCurrency() : 40308;
    return getInventory().consumeItem(cur, amount);
}
/** 依自身商店幣別收款（私商店用） */
public void gainShopCurrency(int amount, l1j.server.server.model.Instance.L1PcInstance seller) {
    int cur = (seller != null) ? seller.getTradeCurrency() : 40308;
    getInventory().storeItem(cur, amount);
}
// --- END: Dual-currency Private Shop fields/methods ---
...
// ● アカウントネーム
	...
	private void levelUp(int gap) {
		...
		// 復活のポーション
		if (getLevel() == 110 && Config.ALT_REVIVAL_POTION&&!getInventory().checkItem(43000,1)) {
			try {
		...
		sendPackets(new S_OwnCharStatus(this));
		l1j.william._Level.getItem(this); //TODO　升級獎勵道具  by 阿傑
...
for (L1ItemInstance item : pc.getInventory().getItems()) {
	...
	// 掛機技能登錄模式旗標
	private boolean _afkSkillRegMode = false;

	public void setAfkSkillReg(boolean flag) {
		_afkSkillRegMode = flag;
	}

	public boolean isAfkSkillReg() {
		return _afkSkillRegMode;
	}



    public int getShopMoneyType() { return _shopMoneyType; }
    public void setShopMoneyType(int t) { _shopMoneyType = t; }


// === PShop currency selection accessors ===
public boolean isShopModeChosen() { return _shopModeChosen; }
public void setShopModeChosen(boolean flag) { _shopModeChosen = flag; }
public boolean isShopModePendingOpen() { return _shopModePendingOpen; }
public void setShopModePendingOpen(boolean flag) { _shopModePendingOpen = flag; }
public void setPrivateShopCurrencyItemId(int itemId) { _privateShopCurrencyItemId = itemId; }
public int getPrivateShopCurrencyItemId() { return _privateShopCurrencyItemId; }
// === end ===
