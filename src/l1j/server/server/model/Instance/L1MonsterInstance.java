...
public class L1MonsterInstance extends L1NpcInstance {
	private transient boolean _afkHiddenSent = false;
	...
	public void onDoppel(boolean isChangeShape) {
		if (getNpcTemplate().is_doppel()) {
			...
			if (!isChangeShape) { // 復原
				...
				isDoppel = true;
				updateObject = true;
				// 變形怪會偽裝成玩家外觀，但為了避免客戶端把它視為「玩家角色」
				// （拖曳道具會觸發交易流程，導致任務/煉化交付失敗），
				// 這裡不再同步玩家名字到 NPC 的 Name/NameId。
				// 外觀(TempCharGfx/GfxId) 仍維持變形效果；顯示名稱維持原 NPC 名稱。
	...
	@Override
	public void onNpcAI() {
        // AFK: if we spawn already hidden (sink/fly), notify once so AFK AI can drop immediately.
        try {
            if (!_afkHiddenSent) {
                int __hs = getHiddenStatus();
                if (__hs == HIDDEN_STATUS_SINK || __hs == HIDDEN_STATUS_FLY) {                   
                    _afkHiddenSent = true;
                }
            }
	...
	private void distributeExpDropKarma(L1Character lastAttacker) {
		...
		if (pc != null) {
			DailyQuestTable.getInstance().onMonsterKilled(pc, getNpcTemplate().get_npcId());
