...
public class L1NpcInstance extends L1Character {
    ...
    // === Priest: 召喚此 NPC 的祭司道具編號 (item_objid) ===
    private int _priestItemObjId = 0;

    public void setPriestItemObjId(int itemObjId) {
        _priestItemObjId = itemObjId;
    }

    public int getPriestItemObjId() {
        return _priestItemObjId;
    }
	...
	public void refineItem() {
		...
		// NOTE:
		// 一部「煙化/試煉」型怪物會透過「給予材料→怪物死亡後掉落相關任務物品/材料」的方式運作。
		// 原始實作在許多配方上都用 (getExp() != 0) 當作啟動條件，
		// 但某些試煉/任務怪物本來就設定為 0 EXP，導致物品不會被煙化，
		// 玩家擊殺後仅會拿回原始給予的材料。
		// 這裡改用「怪物存活」作為安全條件，確保任務怪也能正常煙化。
		if (_npcTemplate.get_npcId() == 45032) { // ブロッブ
			// オリハルコンソードの刀身
			if ((!isDead()) && !_inventory.checkItem(20)) {
			...
			// ロングソードの刀身
			if ((!isDead()) && !_inventory.checkItem(19)) {
			...
			// ショートソードの刀身
			if ((!isDead()) && !_inventory.checkItem(3)) {
				materials = new int[] { 40494, 40521 };
				counts = new int[] { 50, 1 };
			...
			// オリハルコンホーン
			if ((!isDead()) && !_inventory.checkItem(100)) {
			...
			// ミスリルホーン
			if ((!isDead()) && !_inventory.checkItem(89)) {
		...
		} else if ((_npcTemplate.get_npcId() == 81069) || _npcTemplate.is_doppel()) { // ドッペルゲンガー（クエスト/一般）
			// ドッペルゲンガーの体液
			// 部分版本中，法師試煉使用的「變形怪」npcId 可能不是 81069，但模板仍標記為 doppel。
			// 為避免交付材料後只掉回原材料，這裡對所有 doppel 類怪物套用同一轉換規則。
			if ((!isDead()) && !_inventory.checkItem(40542)) {
			...
			// パンプキンの種
			if ((!isDead()) && !_inventory.checkItem(40726)) {
