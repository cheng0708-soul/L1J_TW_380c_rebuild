...
import l1j.server.server.priest.PriestBuffIconResolver;
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_skill.getTarget().equals("attack") && (_skillId != 18)) {
				...
				// _target 有可能為 null（例如範圍技沒有單一目標或目標已死亡），
				// 為避免 NullPointerException，先檢查再決定要用誰當特效顯示對象。
				int targetid;
				if (_target != null) {
					targetid = _target.getId();
				} else {
					// 沒有具體目標時，就以施法 NPC 自己當作特效中心。
					...
					case BLESSED_ARMOR:
                        if (cha instanceof L1PcInstance) {
                            ...
                            if ((item != null) && (item.getItem().getType2() == 2) && (item.getItem().getType() == 2)) {
                                ...
                                // 設定角色身上的技能效果（供 AI 與 UI 使用）
                                pc.setSkillEffect(BLESSED_ARMOR, _skill.getBuffDuration() * 1000);
                                // 推送對應圖示（Shield 類）
                                PriestBuffIconResolver.sendIcon(pc, BLESSED_ARMOR, _skill.getBuffDuration());
                            }
                            else {
                                pc.sendPackets(new S_ServerMessage(79));
                            }
