	...
	public static int skillEffect(L1Character _user, L1Character cha, L1Character _target, int skillId, int _getBuffIconDuration, int dmg) {
		...
		case CANCELLATION:
			...
			if (cha instanceof L1PcInstance) {
				...
				// 強化系の能力値アイコンをサーバー側の効果と同期させる
				// 相消術で効果が解除されているのにクライアント側のアイコンだけ残る現象を防ぐ

				// STR 系（體魄強健術 / 力量上升、變身裝備 DRESS_MIGHTY 等）
				if (!pc.hasSkillEffect(PHYSICAL_ENCHANT_STR)
						&& !pc.hasSkillEffect(DRESS_MIGHTY)) {
					// どの STR 系バフも掛かっていないので、関連アイコンをまとめて消す
					pc.sendPackets(new S_Strup(pc, 5, 0));
					pc.sendPackets(new S_Strup(pc, 2, 0));
				}

				// DEX 系（通暢氣脈術 / 敏捷上升、變身裝備 DRESS_DEXTERITY 等）
				if (!pc.hasSkillEffect(PHYSICAL_ENCHANT_DEX)
						&& !pc.hasSkillEffect(DRESS_DEXTERITY)) {
					// どの DEX 系バフも掛かっていないので、関連アイコンをまとめて消す
			...
			if (cha instanceof L1PcInstance) {
				...
				// バフ一覧を再送信してクライアント側の状態欄を強制リフレッシュ
