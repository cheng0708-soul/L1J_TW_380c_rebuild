...
public class L1CastleLocation {
	...
	/**
	 * 共通：任意座標がいずれかの戦争エリア（旗内）かどうかをチェック
	 */
	public static boolean isInAnyWarArea(L1Location loc) {
		return getCastleIdByArea(loc) != 0;
	}

	/**
	 * 共通：プレイヤーがいずれかの戦争エリア（旗内）にいるかどうかをチェック
	 */
	public static boolean isInAnyWarArea(L1PcInstance pc) {
		if (pc == null) {
			return false;
		}
		return isInAnyWarArea(pc.getLocation());
	}

	/**
	 * 戦争エリアからプレイヤーを安全な場所へテレポートさせる。
	 * 対象が戦争エリア外にいる場合は何もしない。
	 */
	public static void teleportOutFromWarArea(L1PcInstance pc) {
		if (pc == null) {
			return;
		}
		int castleId = getCastleIdByArea(pc.getLocation());
		if (castleId == 0) {
			return;
		}
		...
		case GIRAN_CASTLE_ID: // ギラン城
			x = GIRAN_X1 - 1;
			y = (GIRAN_Y1 + GIRAN_Y2) / 2;
			mapId = GIRAN_MAP;
			break;
		case HEINE_CASTLE_ID: // ハイネ城
			x = HEINE_X1 - 1;
			y = (HEINE_Y1 + HEINE_Y2) / 2;
			mapId = HEINE_MAP;
			break;
	...
	/**
	 * 戦争エリア内で魔法娃娃を所持しているプレイヤーを検知して外へ出す。
	 */
	public static void checkMagicDollInWarArea(L1PcInstance pc) {
		if (pc == null) {
			return;
		}
		if (pc.getDollList() == null || pc.getDollList().isEmpty()) {
			return;
		}
		if (!isInAnyWarArea(pc)) {
			return;
		}
		teleportOutFromWarArea(pc);
	}
	/**
