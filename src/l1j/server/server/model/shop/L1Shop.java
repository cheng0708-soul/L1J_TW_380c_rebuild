...
public class L1Shop {
    private final int _shopNpcId; // NPC id of this shop

    private final int _currencyItemId; // 結算貨幣（Adena 或 元寶 240107）
	...
	public L1Shop(int npcId, List<L1ShopItem> sellingItems,
        ...
        this._currencyItemId = (npcId == 970001) ? 240107 : L1ItemId.ADENA;
	...
	/**
	 * 稅金免除商店判定：目前以「結算貨幣不是金幣(ADENA)」為準。
	 * 主要用於商城管理員（商城系統），讓其販賣價格不受城鎮/城堡稅率影響。
	 */
	public boolean isTaxFreeShop() {
		return _currencyItemId != L1ItemId.ADENA;
	}
	...
	private boolean ensureSell(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		// 購入できるかチェック
		if (!pc.getInventory().checkItem(_currencyItemId, price)) { // \f1アデナが不足しています。
			if (_shopNpcId == 970001) { pc.sendPackets(new S_SystemMessage("元寶不足")); } else { pc.sendPackets(new S_ServerMessage(189)); }
			return false;
		}
		// 重量チェック
	...
	private void payTax(L1ShopBuyOrderList orderList) {
		// 商城管理員（商城系統）等使用非金幣結算的商店，不計算任何稅金。
		if (isTaxFreeShop()) {
			return;
		}
