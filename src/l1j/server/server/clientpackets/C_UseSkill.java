...
try {
    ...
    try {
        ...
        java.lang.reflect.Method mm = svc.getMethod("captureSkillFromCast", pcClazz, int.class);
        mm.invoke(null, pc, skillId);
    } catch (Throwable ignore) {}
} catch (Throwable ignore) {}
// 跟隨協同系統：輔助魔法登錄捕捉（不影響當次施法）
try {
    l1j.server.server.afk.CoopFollowService.captureSupportSkillFromCast(pc, skillId, targetId);
} catch (Throwable ignore) {}
