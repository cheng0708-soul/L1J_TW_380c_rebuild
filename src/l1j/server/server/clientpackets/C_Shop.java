...
// Reset currency selection status on every open attempt
pc.setShopModeChosen(false);
pc.setShopModePendingOpen(false);
 // 開始
...
// Require currency selection before opening; show dialog and stop here
if (!pc.isShopModeChosen()) {
    pc.setShopModePendingOpen(true);
    // send both objId=0 and player's id for client compatibility
    pc.sendPackets(new S_NPCTalkReturn(0, "pshop_mode"));
    pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "pshop_mode"));
    return;
}
