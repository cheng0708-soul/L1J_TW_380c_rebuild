...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		// Sell-all: 登錄模式時，背包點擊視為登錄目標，不執行原本使用效果
		if (l1j.server.server.model.SellAllService.interceptItemClick(pc, l1iteminstance)) {
			return;
		}
        ...
        try {
         ...
         // ★ 全服 debug：不管用什麼道具都印出來
            // === BOSS 重生查詢道具（240142） ===
            if (itemId == 240142) {
				// 改為走 BossStatusService（支援分頁）
				l1j.server.server.model.BossStatusService.open(pc, 0);
                return;
            }
		...
		// === 240105 三段加速(永久) — 40018 + 40733 + 第三段；不消耗且位置不變；三段時效 1800 秒（第三段分段顯示） ===
		if (l1iteminstance.getItem().getItemId() == 240105) {
			try {
				// 綠水、勇敢會各扣 1；先 +2 抵銷
				final int __c0 = l1iteminstance.getCount();
				try { l1iteminstance.setCount(__c0 + 2); } catch (Throwable __e0) {}

				// ① 第一段（強化自我加速）
				try { l1j.server.server.model.item.action.Potion.useGreenPotion(pc, l1iteminstance, 40018); } catch (Throwable __e1) {}

				// ② 第二段（勇敢）
				try { l1j.server.server.model.item.action.Potion.Brave(pc, l1iteminstance, l1j.server.server.model.identity.L1ItemId.COIN_OF_REPUTATION); } catch (Throwable __e2) {}

				// ③ 第三段：手動 1800 秒 + 圖示倒數分兩段（1020s + 780s），避免 UI 上限
				try {
					final int THIRD = l1j.server.server.model.skill.L1SkillId.STATUS_THIRD_SPEED;
					final int D1800 = 1800 * 1000;
					pc.killSkillEffectTimer(THIRD);
					pc.setSkillEffect(THIRD, D1800);
					// 動畫與音效（與蛋糕一致）
					pc.sendPackets(new l1j.server.server.serverpackets.S_Liquor(pc.getId(), 8));
					pc.broadcastPacket(new l1j.server.server.serverpackets.S_Liquor(pc.getId(), 8));
					pc.sendPackets(new l1j.server.server.serverpackets.S_SkillSound(pc.getId(), 7976));
					pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillSound(pc.getId(), 7976));
					// 第三段圖示：客戶端一包最多 255*4=1020 秒，所以先 1020，再補 780
					pc.sendPackets(new l1j.server.server.serverpackets.S_SkillIconThirdSpeed(255)); // 1020 秒
					final l1j.server.server.model.Instance.L1PcInstance __pc = pc;
					l1j.server.server.GeneralThreadPool.getInstance().schedule(new java.lang.Runnable() {
						public void run() {
							try {
								if (__pc != null && __pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_THIRD_SPEED)) {
									__pc.sendPackets(new l1j.server.server.serverpackets.S_SkillIconThirdSpeed(195)); // 780 秒
								}
							} catch (Throwable __et) { }
						}
					}, 1020 * 1000);
				} catch (Throwable __e3) {}

				// ④ 同步第一段/第二段時間到 1800（並更新 UI）
				try {
					final int D1800 = 1800 * 1000;
					// Haste
					if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.GREATER_HASTE)) {
						pc.killSkillEffectTimer(l1j.server.server.model.skill.L1SkillId.GREATER_HASTE);
						pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.GREATER_HASTE, D1800);
						pc.sendPackets(new l1j.server.server.serverpackets.S_SkillHaste(pc.getId(), 1, 1800));
						pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillHaste(pc.getId(), 1, 0));
					} else if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_HASTE)) {
						pc.killSkillEffectTimer(l1j.server.server.model.skill.L1SkillId.STATUS_HASTE);
						pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_HASTE, D1800);
						pc.sendPackets(new l1j.server.server.serverpackets.S_SkillHaste(pc.getId(), 1, 1800));
						pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillHaste(pc.getId(), 1, 0));
					}
					// Brave（依現有型態）
					if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE)) {
						pc.killSkillEffectTimer(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE);
						pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE, D1800);
						pc.sendPackets(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 1, 1800));
						pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 1, 0));
						pc.setBraveSpeed(1);
					} else if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_ELFBRAVE)) {
						pc.killSkillEffectTimer(l1j.server.server.model.skill.L1SkillId.STATUS_ELFBRAVE);
						pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_ELFBRAVE, D1800);
						pc.sendPackets(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 3, 1800));
						pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 3, 0));
						pc.setBraveSpeed(3);
					} else if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE2)) {
						pc.killSkillEffectTimer(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE2);
						pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_BRAVE2, D1800);
						pc.sendPackets(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 7, 1800));
						pc.broadcastPacket(new l1j.server.server.serverpackets.S_SkillBrave(pc.getId(), 7, 0));
						pc.setBraveSpeed(7);
					}
				} catch (Throwable __e4) {}

				// 還原數量（不消耗、欄位不變）
				try { l1iteminstance.setCount(__c0); } catch (Throwable __e5) {}
			} catch (Throwable __e) { /* ignore */ }
			return;
		}

		// === 240104 古代終極體力恢復劑(永久) — 等同 40024；不消耗且位置不變 ===
		if (l1iteminstance.getItem().getItemId() == 240104) {
			try {
				final int __c0 = l1iteminstance.getCount();
				try { l1iteminstance.setCount(__c0 + 1); } catch (Throwable __e0) {}
				l1j.server.server.model.item.action.Potion.UseHeallingPotion(pc, l1iteminstance, 55, 197);
				try { l1iteminstance.setCount(__c0); } catch (Throwable __e1) {}
			} catch (Throwable __e) { /* ignore */ }
			return;
		}
		...
		// === 40117 主城傳送卷：數量不變 + 固定座標 + 使用後必停掛 ===
		if (l1iteminstance.getItemId() == 40117) {
			try {
				final int x = 33082, y = 33389; final short map = 4;

				// ① 記錄使用前的張數（作為“數量不變”基準）
				int __before = 0;
				try { __before = pc.getInventory().countItems(40117); } catch (Throwable __eB) {}

				// ② 傳送到固定座標
				l1j.server.server.model.L1Teleport.teleport(pc, x, y, map, 5, true);

				// ③ 先用「回城停掛」；再補一個「死亡停掛」作為保險——確保總開關一定被關
				try { l1j.server.server.afk.AfkEventBridge.onTownWarp(pc); } catch (Throwable __e1) {}
				try { l1j.server.server.afk.AfkEventBridge.onDeath(pc); } catch (Throwable __e2) {}

				// ④ 數量正規化：保證最終「不消耗也不增加」→ 維持『使用前張數』
				int __after = 0;
				try { __after = pc.getInventory().countItems(40117); } catch (Throwable __eA) {}
				int __delta = __before - __after;
				if (__delta > 0) {
					// 被扣了幾張，就補回幾張
					try { pc.getInventory().storeItem(40117, __delta); } catch (Throwable __eS) {}
				} else if (__delta < 0) {
					// 多出幾張，就移除幾張（避免變成越用越多）
					int __surplus = -__delta;
					try { pc.getInventory().removeItem(40117, __surplus); } catch (Throwable __eR) {
						// 若沒有 removeItem(itemId, count) 多載，就退而求其次直接改實體數量
						try { l1iteminstance.setCount(l1iteminstance.getCount() - __surplus); } catch (Throwable __eR2) {}
					}
				}
			} catch (Throwable __e) { /* ignore */ }
			return; // 截斷後續通用流程，避免又被其它分支處理
		}

		// === 240103 自訂傳送卷：數量不變 + 固定座標 + 使用後必停掛 ===
				// === 240103 自訂傳送卷：數量不變 + 固定座標 + 使用後必停掛（加強版） ===
		if (l1iteminstance.getItemId() == 240103) {
			try {
				final int x = 33082, y = 33389; final short map = 4;

				// ① 記錄使用前張數（不消耗基準）
				int __before = 0;
				try { __before = pc.getInventory().countItems(240103); } catch (Throwable __eB) {}
				// ---- 強制停掛（多重保險）BEGIN ----
				try { l1j.server.server.afk.AfkController.forceOff(pc); } catch (Throwable __eF1) {}
				try { l1j.server.server.afk.AfkCruiseEngine.stop(pc); } catch (Throwable __eF2) {}
				try { l1j.server.server.afk.AfkService.onDisconnect(pc); } catch (Throwable __eF3) {}
				try { l1j.server.server.afk.AfkMagicRegistry.setEnabled(pc, false); } catch (Throwable __eF4) {}
				// ---- 強制停掛（多重保險）END ----
				...
				// ④ 不消耗正規化
				int __after = 0;
				try { __after = pc.getInventory().countItems(240103); } catch (Throwable __eA) {} 
				int __delta = __before - __after;
				if (__delta > 0) {
					try { pc.getInventory().storeItem(240103, __delta); } catch (Throwable __eS) {}
				} else if (__delta < 0) {
					int __surplus = -__delta;
					try { pc.getInventory().removeItem(240103, __surplus); } catch (Throwable __eR) {
						try { l1iteminstance.setCount(l1iteminstance.getCount() - __surplus); } catch (Throwable __eR2) {} 
					}
				}
			} catch (Throwable __e) { /* ignore */ }
...
else if (l1iteminstance.getItemId() == 240141) {
    try {
        final int x = 32800, y = 32927; final short map = 800;
        final int __beforeFinal = pc.getInventory().countItems(240141);
        try { l1j.server.server.afk.AfkController.forceOff(pc); } catch (Throwable __e1) {}
        try { l1j.server.server.afk.AfkCruiseEngine.stop(pc); } catch (Throwable __e2) {}
        try { l1j.server.server.afk.AfkService.onDisconnect(pc); } catch (Throwable __e3) {}
        try { l1j.server.server.afk.AfkMagicRegistry.setEnabled(pc, false); } catch (Throwable __e4) {}
        l1j.server.server.model.L1Teleport.teleport(pc, x, y, map, 5, true);
        l1j.server.server.GeneralThreadPool.getInstance().schedule(new java.lang.Runnable() {
            @Override
            public void run() {
                try {
                    int __afterFinal = pc.getInventory().countItems(240141);
                    int __need = __beforeFinal - __afterFinal;
                    if (__need > 0) {
                        pc.getInventory().storeItem(240141, __need);
                    }
    ...
    return;
		...
		switch (itemId) {
			...
			case 49144: case 49145: case 49146: case 49147:
			case 47048: case 47049: case 47050: case 47051: case 47052: case 49198: case 49199: case 240128:
			case 240129: case 240134: case 240135:  case 240130: case 240131: case 240136: // growth & IQ potions (choice)
l = readD();
				break;
			...
			case 41293: case 41294: // 釣竿
                ...
                // AutoFarm Controller (custom) - 240101
                case 240101: { // 掛機控制器
                    try {
                        l1j.server.server.model.item.function.AutoFarmToggle.get()
                            .execute(0, pc, l1iteminstance); // 不消耗道具
                    } catch (Throwable t) {
                        pc.sendPackets(new l1j.server.server.serverpackets.S_SystemMessage(
                            "AutoFarmToggle not available: " + t.getClass().getSimpleName()));
                    }
                    return; }

                // AutoFarm Controller direct-open item - 30652
                case 30652: { // 掛機控制器(直開)
                    try {
                        l1j.server.server.model.item.function.AutoFarmToggle.get()
                            .execute(0, pc, l1iteminstance); // 不消耗道具
                ...
                // One-click teleport item - 30653
                case 30653: {
                    try {
                        // Reuse the existing tracy1 dialog shell to avoid adding a new client HTML name.
                        pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "tracy1"));
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("Teleport dialog not available: " + t.getClass().getSimpleName()));
                    }
                    return; }

    
                // Portable Shop item - 240151
                case 240151: {
                    try {
                        pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "portable"));
                    } catch (Throwable t) {
					pc.sendPackets(new S_SystemMessage("Portable shop not available: " + t.getClass().getSimpleName()));
                    }
                    return; }


                // Coop Follow Controller item - 240152
                case 240152: {
                    try {
                        l1j.server.server.afk.CoopFollowService.openMainUi(pc);
                    } catch (Throwable t) {
                        pc.sendPackets(new l1j.server.server.serverpackets.S_SystemMessage(
                            "CoopFollowService not available: " + t.getClass().getSimpleName()));
                        try {
                ...
                // One-click sell item - 240153
                case 240153: {
                    try {
                        pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "sell_all"));
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("Sell-all dialog not available: " + t.getClass().getSimpleName()));
                    }
                    return; }

                // AFK Recharge items (custom) - 800022/800023/800024
                case 800022: { // 掛機充值(10分鐘)
                    try {
                        l1j.server.server.model.item.function.AfkRecharge10m.get().execute(0, pc, l1iteminstance);
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("AfkRecharge10m not available: " + t.getClass().getSimpleName()));
                    }
                    return; }
                case 800023: { // 掛機充值(30分鐘)
                    try {
                        l1j.server.server.model.item.function.AfkRecharge30m.get().execute(0, pc, l1iteminstance);
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("AfkRecharge30m not available: " + t.getClass().getSimpleName()));
                    }
                    return; }
                case 800024: { // 掛機充值(60分鐘)
                    try {
                        l1j.server.server.model.item.function.AfkRecharge60m.get().execute(0, pc, l1iteminstance);
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("AfkRecharge60m not available: " + t.getClass().getSimpleName()));
                    }
                    return; }
                // AFK Upgrade item (custom) - 800030
                case 800030: { // 掛機升級(高級掛機)
                    try {
                        l1j.server.server.model.item.function.AfkUpgradeHighGrade.get().execute(0, pc, l1iteminstance);
                    } catch (Throwable t) {
                        pc.sendPackets(new S_SystemMessage("AfkUpgradeHighGrade not available: " + t.getClass().getSimpleName()));
                    }
                    return; }
...
default:
		...
		if (pc.getCurrentHp() > 0) {
            ...
            // === Custom: Princess/Priest growth + Priest IQ (item-to-item choice) ===
            if (itemId == 240136) { // Priest IQ Potion
                try {
                    l1j.server.server.model.item.function.PriestIQPotion2.get().execute(l, pc, l1iteminstance);
                } catch (Throwable t) {
                    pc.sendPackets(new l1j.server.server.serverpackets.S_SystemMessage("祭司智力成長藥劑異常：" + t.getClass().getSimpleName()));
                }
                return;
            }
            else if (itemId == 240131) { // Priest Growth Potion
                try {
                    l1j.server.server.model.item.function.PriestGrowthPotion.get().execute(l, pc, l1iteminstance);
                } catch (Throwable t) {
                    pc.sendPackets(new l1j.server.server.serverpackets.S_SystemMessage("祭司成長藥劑異常：" + t.getClass().getSimpleName()));
                }
                return;
            }
            else if (itemId == 240130) { // Princess Growth Potion
                try {
                    l1j.server.server.model.item.function.PrincessGrowthPotion.get().execute(l, pc, l1iteminstance);
                } catch (Throwable t) {
                    pc.sendPackets(new l1j.server.server.serverpackets.S_SystemMessage("公主娃娃成長藥劑異常：" + t.getClass().getSimpleName()));
                }
                return;
            }
            // === End Custom ===
if ((itemId == 40077) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_WEAPON) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON)
					...
					|| (itemId == L1ItemId.C_SCROLL_OF_ENCHANT_WEAPON) || (itemId == 40128) || (itemId == 240128)) { // 對武器施法的卷軸
				...
				Enchant.scrollOfEnchantWeaponIvoryTower(pc, l1iteminstance, l1iteminstance1, client);
			        ...
			        || itemId == 49144 || itemId == 49145 || itemId == 49146 || itemId == 49147) {
			    Enchant.scrollOfEnchantWeaponAttr(pc, l1iteminstance, l1iteminstance1, client);
					...
					|| (itemId == L1ItemId.B_SCROLL_OF_ENCHANT_ARMOR) || (itemId == L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR) || (itemId == 40127) || (itemId == 240129)) { // 對盔甲施法的卷軸
			...
			else if (itemId == 240134 || itemId == 240135) { // 自訂：防爆卷
				// 目標必須存在，且為武器(type2=1)或盔甲(type2=2)
				if (l1iteminstance1 == null) {
					pc.sendPackets(new S_ServerMessage(79)); // 沒有任何事情發生
					return;
				}
				L1ItemInstance targetItem = l1iteminstance1;
				int targetType2 = targetItem.getItem().getType2();
				if (targetType2 != 1 && targetType2 != 2) {
					pc.sendPackets(new S_ServerMessage(79));
					return;
				}

				// 設定防爆類型：1 = 歸0防爆, 2 = 完全防爆
				if (itemId == 240134) {
					targetItem.setEnchantProtectType(1);
				} else { // 240135
					targetItem.setEnchantProtectType(2);
				}
			    ...
			    // ★ 我幫你插在這裡 ★
			    // 自訂：個人轉運符(一小時) - 個人掉寶倍率 +1 倍
			    if (itemId == 240133) {
			        int time = 3600; // 秒
			        if (pc.hasSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_PERSONAL_LUCKY_CHARM)) {
			            pc.sendPackets(new S_SystemMessage("你已經受到個人轉運符的效果影響。"));
			            return;
			        }
			        pc.setSkillEffect(l1j.server.server.model.skill.L1SkillId.STATUS_PERSONAL_LUCKY_CHARM, time * 1000);
			        pc.sendPackets(new S_SkillIconGFX(5647, time)); // 圖示 5647，倒數 3600 秒
			        pc.sendPackets(new S_SystemMessage("個人轉運符生效中，一小時內個人掉寶倍率 +1 倍。"));
			        pc.getInventory().removeItem(l1iteminstance, 1);
			        return;
			    }
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
					...
					if (pc.getBaseStr() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
					...
					if (pc.getBaseCon() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
					...
					if (pc.getBaseDex() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
					...
					if (pc.getBaseInt() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
					...
					if (pc.getBaseWis() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
					...
					if (pc.getBaseCha() < Config.BONUS_STATS3 && pc.getElixirStats() < Config.BONUS_STATS2) { // 調整能力值上限 
...
// TODO: Could not auto-locate 240101 block
