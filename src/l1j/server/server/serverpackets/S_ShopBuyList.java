...
public class S_ShopBuyList extends ServerBasePacket {
	...
	public S_ShopBuyList(int objid, L1PcInstance pc) {
		...
		if (object instanceof L1NpcInstance) {
			...
			// Portable shop: use configured NPC shop id
    ...
    // 安全模式：空清單但不閃退
    pc.sendPackets(new l1j.server.server.serverpackets.S_ServerMessage(79)); // 沒有可出售的物品
    writeC(l1j.server.server.Opcodes.S_OPCODE_SHOWSHOPSELLLIST);
    writeD(objid);
    writeH(0);
    return;
}
