...
public class S_ShopSellList extends ServerBasePacket {
	...
	public S_ShopSellList(int objId, L1PcInstance pc) {
		...
		if (npcObj instanceof L1NpcInstance) {
			npcId = ((L1NpcInstance) npcObj).getNpcTemplate().get_npcId();
		} else if (objId == PortableShop.PORTABLE_OBJID) {
			// Portable shop: use configured NPC shop (e.g. 90000)
			npcId = PortableShop.PORTABLE_NPC_ID;
		} else {
			writeH(0);
			return;
		}
		...
		for (int i = 0; i < shopItems.size(); i++) {
			...
			int basePrice = (int) (shopItem.getPrice() * Config.RATE_SHOP_SELLING_PRICE);
			int price = shop.isTaxFreeShop() ? basePrice : calc.layTax(basePrice);
			writeD(i);
