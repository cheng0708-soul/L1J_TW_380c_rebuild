...
import l1j.server.server.templates.L1MagicDoll; // ★ 新增
...
public class CalcExp {
	...
	private static void AddExp(L1PcInstance pc, long exp, int lawful) {
        ...
        // 魔法娃娃經驗加成：使用 magic_doll.exp_percent，150→1.5倍、200→2.0倍...
        int dollExpPercent = L1MagicDoll.getExpPercentByDoll(pc);
        if (dollExpPercent > 0) {
            dollBonus = dollExpPercent / 100.0D; // 150→1.5, 200→2.0, 350→3.5
        } else {
            dollBonus = 1.0D;
        }
        long add_exp = (long) (exp * exppenalty * Config.RATE_XP * foodBonus * expBonus * ainBonus * dollBonus);

		// TODO 暴等洗血修正
		if (add_exp < 0) {//TODO 經驗值大於2147483647會變負值導致錯誤.
			add_exp = 0;
		} else if (add_exp > 36065092) {//TODO 升一級所需要的EXP=36065092
			add_exp = 36065092;
		}
		pc.addExp(add_exp);
		pc.addMonsKill();
	...
	private static void AddExpPet(L1PetInstance pet, long exp) {
		...
		//TODO 寵物經驗倍率
		long totalExp = (long) (exp * Config.RATE_XP_PET + pet.getExp());
		//TODO 寵物經驗倍率
